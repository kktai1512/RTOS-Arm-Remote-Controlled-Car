...
void initUltrasonic2(void) {
    ...
    //set port a to timer mode (input capture)
    PORTA->PCR[ECHO_IN2] &= ~PORT_PCR_MUX_MASK;
    PORTA->PCR[ECHO_IN2] |= PORT_PCR_MUX(3);

    //set to GPIO mode
    PORTD->PCR[TRIG_OUT2] &= ~PORT_PCR_MUX_MASK;
    PORTD->PCR[TRIG_OUT2] |= PORT_PCR_MUX(1);

    PTA->PDDR &= ~MASK(ECHO_IN2);
    PTD->PDDR |= MASK(TRIG_OUT2);
...
void pulse(void) {
 ...
 void pulse2(void) {
	PTD->PSOR |= MASK(TRIG_OUT2);
	//detectEcho();
	delay(uS(10));
	PTD->PCOR |= MASK(TRIG_OUT2);
}
